...
x <- tibble(p_IB = map_int(1:n, ~ roll_two_dice()),
  ...
  mutate(winner = "Tie") %>% 
  mutate(winner = ifelse(IB_craps & !JM_craps, "IB", winner)) %>% 
  mutate(winner = ifelse(JM_craps & !IB_craps, "JM", winner)) %>% 
...
```{r}
# this looks at how many times he could have run in a number of contests. In other words, JM won 0 times in 347 contests
table(x$jm_wins)
